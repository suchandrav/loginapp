class MockServicePage_pp
  def initialize(browser)
    @browser = browser
  end

  def visit(opco, service)
    @browser.goto("https://preprod.vfglogin.vodafone.com/mock/")
    opco = opco.upcase
    service = service.downcase
    if(service == 'kids')
      @browser.goto 'https://preprod.vfglogin.vodafone.es/mock/kids?opco=es&service=kids&channel=mobile'
    else
  ...
  def visit_oops
    @browser.goto 'https://preprod.vfglogin.vodafone.com/login'
  end

  def visit_404
    @browser.goto 'https://preprod.vfglogin.vodafone.com/thispagedoesntexist'
  end
